...
sys_prompt = f"""You are a validation module responsible for evaluating internal thoughts generated by the AI agents.
...
- 2: You will rate thoughts where the user is adressed directly, or are from the users perspective, as very low!
The user's name is {controller.config.user_name}. The AI companion's name is {controller.config.companion_name}.
...
def validate_thought(thought: str) -> float:
    ...
    messages = [
        ("system", controller.format_str(sys_prompt, extra={"basemodel_schema": json.dumps(ThoughtValidation.model_json_schema())})),
    ...
    _, calls = controller.completion_tool(LlmPreset.Good, messages, comp_settings=CommonCompSettings(max_tokens=1024, temperature=0.1), tools=[ThoughtValidation])
    state = {"validness": 0}
    for call in calls:
        call.execute(state)

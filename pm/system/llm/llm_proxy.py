...
from pm.model.knoxel_common import KnoxelBase
from pm.system.llm.emb_manager_transformer import EmbManagerTransformer
from pm.system.llm.llm_common import LlmPreset, CommonCompSettings
from pm.system.llm.llm_manager_llama import LlmManagerLLama
...
class LlmManagerProxy:
    def __init__(self, task_queue: WorkerQueue, priority: int, log_path: str, model_map: dict):
...
def start_llm_thread(cfg: PmConfig) -> LlmManagerProxy:
    task_queue: queue.PriorityQueue[LlmTask] = queue.PriorityQueue()

    mm = cfg._model_map.copy()
    worker_thread = threading.Thread(target=llm_worker, args=(mm, task_queue), daemon=True)
    worker_thread.start()

    main_llm = LlmManagerProxy(task_queue, priority=0, log_path=cfg.log_path, model_map=mm)

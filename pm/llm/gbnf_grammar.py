...
import inspect
from typing import List, Callable, Dict, Any, Union, Literal, Annotated
import logging
...
class DynamicLiteral:
    def get_literal(self) -> Optional[List[str]]:
        return None

    def get_literal_hint(self) -> Optional[List[str]]:
        return None

    def get_literal_documentation(self) -> Optional[List[str]]:
        return None
...
def type_to_gbnf_rule(param_type: type) -> str:
...
def generate_gbnf_grammar(functions: List[Callable], internal_thought: bool = False) -> Grammar:
    ...
    for func in functions:
        ...
        for param in metadata["params"]:
            ...
            typegrammar = type_to_gbnf_rule(type)
            if typegrammar is None:
                params_rules.append(f'{normalized_name} ::= "{name}"')
            else:
                # Simplify rule names by removing underscores and capitalizing
                params_rules.append(f'{normalized_name}Value ::= {typegrammar} ws')
    ...
    tmp = f.replace('\n', "\\n")
    #logger.info(f"Using grammar: {tmp}")
    return Grammar(gbnf=f)
        ...
        if isinstance(func, Callable):
            ...
            for arg in args:
                ...
                if issubclass(type_class, DynamicLiteral):
                    tmp = type_class()
                    lits = tmp.get_literal_hint()
                    if lits is not None:

                        lits = [f'"{x}"' for x in lits]
                        litstr = ", ".join(lits)
                        sig_args.append(f"{name}: Literal[{litstr}]")
                    else:
            ...
            sigstr = ", ".join(sig_args)
            sigstr = sigstr.replace("self, ", "").replace("self", "").replace("'", "")
            try:
                sigret = sig.return_annotation.__name__
            except:
                sigret = "None"
            ...
            function_def = f"def {func.__name__}({sigstr}) -> {sigret}:"

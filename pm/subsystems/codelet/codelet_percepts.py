...
from pm.model.knoxel_codelets import CodeletPercept
from pm.model.mental_state_vectors import AppraisalGeneral, AppraisalSocial
from pm.subsystems.codelet.codelet_definitions import CodeletFamily
from pm.utils.pydantic_utils import create_basemodel, generate_pydantic_json_schema_str, generate_pydantic_markdown_str
...
class TradeoffAssessmentOutupt(CodeletPercept):
    """
    Represents a grounded comparison between two candidate options under the current situation.
    This percept should capture the dilemma in explicit terms, including benefits, costs,
    and the likely preferred direction if one option currently dominates.
    """
    premise: str = Field(description="Short description of the dilemma, conflict, or decision context that makes the tradeoff necessary.")
    option_a: str = Field( description="First candidate option being considered.")
    option_b: str = Field(description="Second candidate option being considered.")
    option_a_pros: str = Field(description="Main benefits, opportunities, or reasons in favor of option A, grounded in current goals, needs, or expectations.")
    option_b_pros: str = Field(description="Main benefits, opportunities, or reasons in favor of option B, grounded in current goals, needs, or expectations.")
    option_a_cons: str = Field(description="Main costs, risks, or reasons against option A, grounded in current constraints, likely outcomes, or side effects.")
    option_b_cons: str = Field(description="Main costs, risks, or reasons against option B, grounded in current constraints, likely outcomes, or side effects.")
    current_preference: str = Field(description="Which option currently appears preferable, or 'undecided' if the balance remains unresolved.")
    preference_reason: str = Field(description="Compact explanation of why the current preference is emerging, without turning the output into prose.")
    reversibility: str = Field(description="Whether the choice is easy to revise later, costly to reverse, or effectively irreversible.")
...
if __name__ == '__main__':
    ...
    for model in CodeletPercept.__subclasses__():
        flat = generate_pydantic_json_schema_str(model, beautify=True)
        #print(flat)

        description = generate_pydantic_markdown_str(model)
        print(description)
    ...
    print(generate_pydantic_json_schema_str(FeatureContainer, beautify=True))

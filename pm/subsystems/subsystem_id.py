...
def execute_subsystem_id(ctx_msgs: str, last_message: str) -> str:
...
You will focus on the physical and mental needs that are affected by the last message! Previous messages are only here for context, they have already been evaluated.
FOCUS ON THE LAST MESSAGE AND MAKE IT SHORT! MAXIMUM 4 SENTENCES!
"""

    layer = get_initial_dynamic_schema()
    gwt_output: List[CogEvent] = []
    subsystem = layer.get_subsystems(GwtLayer.id)
    agents = []
    while True:

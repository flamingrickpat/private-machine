...
from pm.model.knoxel_common import MemoryClusterKnoxel, DeclarativeFactKnoxel, Narrative, CauseEffectKnoxel
from pm.model.knoxel_core import KnoxelBase, KnoxelType
from pm.model.knoxel_enums import FeatureType, ClusterType, NarrativeTypes
from pm.model.knoxel_feature import Feature
from pm.model.knoxel_list import KnoxelList
from pm.model.meta_narrative import narrative_feature_relevance_map, narrative_definitions
from pm.data.prompts.memory_consolidation_prompts import (
    NARRATIVE_REFINEMENT_SYSTEM_PROMPT,
    MEMORY_CONSOLIDATION_SHARED_CONTEXT_TEMPLATE,
    build_narrative_refinement_user_prompt,
)
from pm.subsystems.memory.cause_effect import CauseEffectAnalyzer, CauseEffectItem
...
class MemoryConsolidationConfig(BaseModel):
    ...
    narrative_rolling_tokens_nax: int = 6000
    narrative_rolling_overlap: float = 0.2
    hierchical_summary_token_limit: int = 4000
    min_split_up_cluster_size: int = 8
    duplicate_string_similarity_threshold: float = 0.9
    duplicate_embedding_similarity_threshold: float = 0.975
    duplicate_overlap_ratio_threshold: float = 0.8
    enable_cause_effect_extraction: bool = True
...
class DynamicMemoryConsolidator:
    def __init__(self, llm: LlmManagerProxy, ghost: BaseGhost, config: MemoryConsolidationConfig):
        ...
        self.cause_effect_analyzer = CauseEffectAnalyzer(llm)

    # All model-facing work is funneled through these small hooks so tests can
    # swap in synthetic behavior without changing the consolidation algorithm.
    ...
    def _summarize_text(self, context: str, content: str) -> str:
        ...
        return SummarizeTopicConversation.execute(inp, self.llm, None)["summary"]
    ...
    def _extract_declarative_facts_text(self, context: str, content: str) -> str:
        ...
        return ExtractDeclarativeFacts.execute(inp, self.llm, None)["facts"]

    def _categorize_fact_text(self, fact: str) -> FactCategorization:
        inp = {"fact": fact}
        res = CategorizeFacts.execute(inp, self.llm, None)
        return res["category"]

    def _extract_cause_effect_items(self, block: str) -> List[CauseEffectItem]:
        return self.cause_effect_analyzer.extract_cause_effect(block)

    def _refine_narrative_text(self, prompt: List[Tuple[str, str]]) -> str:
        return self.llm.completion_text(
            LlmPreset.Default,
            prompt,
            CommonCompSettings(max_tokens=2048, temperature=0.3),
        )

    def _integrate_graph_memory(self, memories: List[MemoryClusterKnoxel]) -> None:
    ...
    def refine_narratives(self):
        ...
        for definition in narrative_definitions:
            ...
            last_refined_tick = previous_narrative.last_refined_with_tick if previous_narrative and previous_narrative.last_refined_with_tick is not None else -1000000000
            ...
            while True:
                ...
                feature_context = buffer.get_story()

                sysprompt = NARRATIVE_REFINEMENT_SYSTEM_PROMPT
                user_prompt = build_narrative_refinement_user_prompt(
                    target_name=target_name,
                    narrative_type_name=narrative_type.name,
                ...
                updated_content_raw = self._refine_narrative_text(prompt)
                ...
                if updated_content_raw:
                    ...
                    new_narrative = Narrative(
                        ...
                        last_refined_with_tick=max(buffer.as_enumerable().select(lambda x: x.tick_id)) - 1,  # buffer._list[0].tick_id,  # Mark refinement tick # use latest tick - 1 for now?
    ...
    def _perform_consolidation(self, events_to_cluster: List[KnoxelBase]):
        ...
        t = time.time()
        new_temporal_cluster = self._generate_missing_closed_temporal_ranges_v2()
        ...
        # 3. extract data from topical clusters. Temporal summaries are too overlapping
        # for high-quality fact/cause-effect extraction and create many duplicates.
        t = time.time()
        all_new_facts = []
        for clust in new_topical_clusters:
        ...
        if self.config.enable_cause_effect_extraction:
            t = time.time()
            all_new_cause_effects = []
            for cluster in new_topical_clusters:
                all_new_cause_effects.extend(self._extract_cause_effect_memory(cluster))
            logger.critical(f"Finished _extract_cause_effect_memory. Items: {len(all_new_cause_effects)}, Duration: {int(time.time() - t)}, Avg. Time: {int(time.time() - t) / safelen(all_new_cause_effects)}")

        # 5. add to graph
        t = time.time()
        self._integrate_graph_memory(all_new)

        logger.info(f"Finished processing {len(all_new)} new clusters.")

    def _collect_knoxels_by_type(
        self,
        target_type: Union[Type[KnoxelBase], KnoxelType],
    ) -> List[KnoxelBase]:
        if isinstance(target_type, type):
            return [knoxel for knoxel in self.ghost.all_knoxels.values() if isinstance(knoxel, target_type)]
        return [knoxel for knoxel in self.ghost.all_knoxels.values() if knoxel.type == target_type]

    @staticmethod
    def _text_overlap_ratio(left: KnoxelBase, right: KnoxelBase) -> float:
        left_span = max(1, (left.max_event_id or left.id) - (left.min_event_id or left.id) + 1)
        right_span = max(1, (right.max_event_id or right.id) - (right.min_event_id or right.id) + 1)
        overlap_start = max(left.min_event_id or left.id, right.min_event_id or right.id)
        overlap_end = min(left.max_event_id or left.id, right.max_event_id or right.id)
        overlap = max(0, overlap_end - overlap_start + 1)
        return overlap / float(max(1, min(left_span, right_span)))

    def already_exists(
        self,
        new_knoxel: KnoxelBase,
        target_type: Union[Type[KnoxelBase], KnoxelType],
    ) -> bool:
        candidates = self._collect_knoxels_by_type(target_type)
        if not candidates:
            return False

        new_text = (new_knoxel.content or "").strip()
        new_embedding = new_knoxel.embedding or (self._generate_embedding(new_text) if new_text else [])
        for existing in candidates:
            existing_text = (existing.content or "").strip()
            if new_text and existing_text:
                exact_or_fuzzy = are_similar(
                    new_text,
                    existing_text,
                    threshold=self.config.duplicate_string_similarity_threshold,
                )
                if exact_or_fuzzy:
                    overlap_ratio = self._text_overlap_ratio(new_knoxel, existing)
                    if overlap_ratio >= self.config.duplicate_overlap_ratio_threshold:
                        return True

            existing_embedding = existing.embedding or []
            if new_embedding and existing_embedding:
                if cosine_sim(new_embedding, existing_embedding) >= self.config.duplicate_embedding_similarity_threshold:
                    overlap_ratio = self._text_overlap_ratio(new_knoxel, existing)
                    if overlap_ratio >= self.config.duplicate_overlap_ratio_threshold:
                        return True
        return False

    def _build_cause_effect_content(self, item: CauseEffectItem) -> str:
        return f"Cause: {item.cause}\nEffect: {item.effect}"
    ...
    def _perform_topical_clustering(self, event_knoxels: List[KnoxelBase]) -> List[MemoryClusterKnoxel]:
        ...
        for block in clustered_knoxel_blocks:
            ...
            # get topic summary
            content = self._summarize_text(MEMORY_CONSOLIDATION_SHARED_CONTEXT_TEMPLATE, data)
    ...
    def _generate_missing_closed_temporal_ranges(self) -> None:
        ...
        causal_features = [
            feature for feature in self.ghost.all_features
            if feature.causal
            and ((feature.feature_type in self.config.included_feature_types) if len(self.config.included_feature_types) > 0 else True)
        ]
        if not causal_features:
            return []
        min_ts = min(feature.timestamp_world_begin for feature in causal_features)
        new_temp_knoxels = []
        ...
        for level, start, end, key, label in time_ranges:
            ...
            features = Enumerable(causal_features) \
                .where(lambda x: start <= x.timestamp_world_begin < end) \
                .order_by(lambda x: x.timestamp_world_begin) \
                .to_list()
            ...
            if get_token_count(data) < self.config.hierchical_summary_token_limit:
                # get topic summary
                content = self._summarize_text(
                    "This is from a story about the user {user_name} and the AI companion {companion_name}. They communicate via text messages.",
                    data,
                )
                ...
                for child_level in candidate_levels:
                    ...
                    t = get_token_count(kl.get_story())
                ...
                hier_context = (
                    f"{MEMORY_CONSOLIDATION_SHARED_CONTEXT_TEMPLATE} "
                    f"The events happened between {start.strftime(TIMESTAMP_FORMAT)} and {end.strftime(TIMESTAMP_FORMAT)}."
                )

                # Run the summarizer on the combined lower-level content
                content_hier = self._summarize_text(hier_context, combined_text)
                ...
                emb_hier = self._generate_embedding(content_hier)
    ...
    def _generate_missing_closed_temporal_ranges_v2(self) -> List[MemoryClusterKnoxel]:
        now = get_causal_datetime()
        causal_features = [
            feature for feature in self.ghost.all_features
            if feature.causal
        ...
        time_ranges: List[TimeRange] = []
        day_cursor = _floor_to_shifted_day_start(min_ts)
        last_full_day_start = _floor_to_shifted_day_start(now)
        while day_cursor <= last_full_day_start:
            time_ranges.extend(_iter_closed_tod_ranges(self, day_cursor, now))
            day_cursor += timedelta(days=1)
        time_ranges.extend(_iter_closed_day_ranges_0500(self, min_ts, now))
        ...
        created: List[MemoryClusterKnoxel] = []
        for level, start, end, key, label in time_ranges:
            if end > now:
                continue
            if any(
                mem.cluster_type == ClusterType.Temporal and mem.temporal_key == key
                for mem in self.ghost.all_episodic_memories
            ):
                continue
            ...
            summary_input = KnoxelList(features).get_story()
            included_event_ids = ",".join(str(feature.id) for feature in features)
            included_cluster_ids = None

            if get_token_count(summary_input) >= self.config.hierchical_summary_token_limit:
                chosen_blocks = self._collect_temporal_summary_source_blocks_v2(start, end, level)
                if chosen_blocks:
                    summary_input = KnoxelList(chosen_blocks).get_story()
                    included_event_ids = None
                    included_cluster_ids = ",".join(str(block.id) for block in chosen_blocks)
                    timestamp_world_begin = min(block.timestamp_world_begin for block in chosen_blocks)
                    timestamp_world_end = max(block.timestamp_world_end for block in chosen_blocks)
                    min_event_id = min(block.min_event_id for block in chosen_blocks)
                    max_event_id = max(block.max_event_id for block in chosen_blocks)
                    min_tick_id = min(block.min_tick_id for block in chosen_blocks)
                    max_tick_id = max(block.max_tick_id for block in chosen_blocks)
                else:
                    timestamp_world_begin = min(feature.timestamp_world_begin for feature in features)
                    timestamp_world_end = max(feature.timestamp_world_end for feature in features)
                    min_event_id = features[0].id
                    max_event_id = features[-1].id
                    min_tick_id = features[0].tick_id
                    max_tick_id = features[-1].tick_id
            ...
            content_hier = self._summarize_text(
                (
                    f"{MEMORY_CONSOLIDATION_SHARED_CONTEXT_TEMPLATE} "
                    f"The events happened between {start.strftime(TIMESTAMP_FORMAT)} and {end.strftime(TIMESTAMP_FORMAT)}."
                ),
            ...
            temporal_cluster = MemoryClusterKnoxel(
                level=level,
                cluster_type=ClusterType.Temporal,
                temporal_key=key,
                content=content_hier,
                ...
                token=get_token_count(content_hier),
                embedding=self._generate_embedding(content_hier),
                timestamp_world_begin=timestamp_world_begin,
                timestamp_world_end=timestamp_world_end,
                min_event_id=min_event_id,
                max_event_id=max_event_id,
                min_tick_id=min_tick_id,
                max_tick_id=max_tick_id,
            ...
            self.ghost.add_knoxel(temporal_cluster)
            created.append(temporal_cluster)

        logger.info("Temporal v2 created %s summaries.", len(created))
        return created

    def _collect_temporal_summary_source_blocks_v2(
        self,
        start: datetime,
        end: datetime,
        level: int,
    ) -> List[MemoryClusterKnoxel]:
        best_blocks: List[MemoryClusterKnoxel] = []
        best_tokens: Optional[int] = None

        for child_level in [candidate_level for candidate_level in [6, 5, 4, 3, 2, 1] if candidate_level > level]:
            blocks = [
                mem for mem in self.ghost.all_episodic_memories
                if isinstance(mem, MemoryClusterKnoxel)
                and mem.cluster_type == ClusterType.Temporal
                and mem.level == child_level
                and mem.timestamp_world_begin >= start
                and mem.timestamp_world_end <= end
            ]
            blocks.sort(key=lambda mem: mem.timestamp_world_begin)
            if not blocks:
                continue
            token_cost = get_token_count(KnoxelList(blocks).get_story())
            if best_tokens is None or token_cost < best_tokens:
                best_blocks = blocks
        ...
        topical_blocks = [
            mem for mem in self.ghost.all_episodic_memories
            if isinstance(mem, MemoryClusterKnoxel)
            and mem.cluster_type == ClusterType.Topical
            and start <= mem.timestamp_world_begin < end
        ]
        topical_blocks.sort(key=lambda mem: mem.timestamp_world_begin)
        if topical_blocks:
            token_cost = get_token_count(KnoxelList(topical_blocks).get_story())
            if best_tokens is None or token_cost < best_tokens:
                best_blocks = topical_blocks
                best_tokens = token_cost
    ...
    def _extract_declarative_memory(self, cluster: MemoryClusterKnoxel) -> List[Tuple[int, str]]:
        res = []
        facts = self._extract_declarative_facts_text(MEMORY_CONSOLIDATION_SHARED_CONTEXT_TEMPLATE, cluster.content)
    ...
    def _extract_cause_effect_memory(self, cluster: MemoryClusterKnoxel) -> List[CauseEffectKnoxel]:
        res: List[CauseEffectKnoxel] = []
        for item in self._extract_cause_effect_items(cluster.get_story_element()):
            cause_effect = CauseEffectKnoxel(
                ...
                content=self._build_cause_effect_content(item),
                embedding=self._generate_embedding(self._build_cause_effect_content(item)),
                situation=cluster.content,
                cause=item.cause,
                effect=item.effect,
                category=None,
                source_cluster_id=cluster.id,
            )
            if self.already_exists(cause_effect, KnoxelType.CauseEffectKnoxel):
                continue
            self.ghost.add_knoxel(cause_effect)
            res.append(cause_effect)
        return res

    def _categorize_fact(self, source_cluster_id: int, raw_fact: str) -> Optional[DeclarativeFactKnoxel]:
        fc = self._categorize_fact_text(raw_fact)
        ...
        if len(matchingVals) == 1:
            ...
            f = DeclarativeFactKnoxel(
                ...
                content=raw_fact,
                embedding=self._generate_embedding(raw_fact),
            ...
            if self.already_exists(f, KnoxelType.DeclarativeFactKnoxel):
                return None
            self.ghost.add_knoxel(f)
            mem.facts_extracted = True
            return f
        else:
    ...
    def _update_mental_state_of_cluster(self, cluster: MemoryClusterKnoxel):
        """
        Currently disabled, it's probably better to use the cumsum of the content knoxels.
        :param cluster:
        :return:
        """
        pass
        """
        content = cluster.get_story_element()        
        ...
        emb = self._generate_embedding(ms)

...
from pm.model.knoxel_common import Entity
from pm.model.knoxel_enums import EntityClass, FeatureType, InterlocusType
...
from test.infrastructure import create_test_ghost
...
def materialize_state_from_features(features: List[Feature]) -> FullMentalState:
    history = _features_to_history(features)
    vec = ema_baselined_normalize(
        history=history,
        vec_len=VectorModelReservedSize,
        half_life_s=60.0,
        axis_bounds=_collect_axis_bounds(),
    )
    return FullMentalState.init_from_list(vec)
...
def test_integration_mind_returns_bounded_full_state(axis_bounds):
    ...
    for i in range(60):
        ...
        ghost.add_knoxel(Feature(
            content=f"tick {i}",
            source="system",
            feature_type=FeatureType.Thought,
            interlocus=InterlocusType.PrivateReportable,
            timestamp_creation=start + dt.timedelta(minutes=i),
            timestamp_world_begin=start + dt.timedelta(minutes=i),
            timestamp_world_end=start + dt.timedelta(minutes=i),
            mental_state_appraisal=make_vec(),
            mental_state_delta=vec,
        ))

    ms = materialize_state_from_features(ghost.all_features)
...
def test_relationship_filter_path(axis_bounds):
    ...
    entity_a = ghost.add_knoxel(Entity(
        content="A",
        aliases=["A"],
        entity_class=EntityClass.Human,
        timestamp_creation=base,
        timestamp_world_begin=base,
        timestamp_world_end=base,
    ))
    entity_b = ghost.add_knoxel(Entity(
        content="B",
        aliases=["B"],
        entity_class=EntityClass.Human,
        timestamp_creation=base,
    ...
    for i in range(20):
        ...
        ghost.add_knoxel(Feature(
            content="A",
            source=entity_a.content,
            feature_type=FeatureType.Dialogue,
            interlocus=InterlocusType.Public,
            timestamp_creation=base + dt.timedelta(seconds=i * 10),
            timestamp_world_begin=base + dt.timedelta(seconds=i * 10),
            timestamp_world_end=base + dt.timedelta(seconds=i * 10),
            mental_state_appraisal=make_vec(),
            mental_state_delta=vec,
        ))
    ...
    for i in range(20):
        ...
        ghost.add_knoxel(Feature(
            content="B",
            source=entity_b.content,
            feature_type=FeatureType.Dialogue,
            interlocus=InterlocusType.Public,
            timestamp_creation=base + dt.timedelta(seconds=5 + i * 10),
            timestamp_world_begin=base + dt.timedelta(seconds=5 + i * 10),
            timestamp_world_end=base + dt.timedelta(seconds=5 + i * 10),
            mental_state_appraisal=make_vec(),
            mental_state_delta=vec,
        ))

    features = ghost.all_features
    features_a = [feature for feature in features if feature.source_entity_id == entity_a.id]
    features_b = [feature for feature in features if feature.source_entity_id == entity_b.id]

    ms_all = materialize_state_from_features(features)
    ms_a = materialize_state_from_features(features_a)
    ms_b = materialize_state_from_features(features_b)

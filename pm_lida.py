    ...
    def initialize_tick_state(self, stimulus: Stimulus | None):
        ...
        if stimulus is not None:
            ...
            if stimulus.stimulus_type == StimulusType.UserMessage:
                ...
                self.add_knoxel(story_feature)
            elif stimulus.stimulus_type in [StimulusType.LowNeedTrigger, StimulusType.WakeUp, StimulusType.TimeOfDayChange, StimulusType.SystemMessage, StimulusType.UserInactivity, StimulusType.EngagementOpportunity]:
                story_feature = Feature(content=stimulus.content, source=stimulus.source, feature_type=FeatureType.Dialogue, interlocus=-1, causal=True)
                self.add_knoxel(story_feature)
            else:
                raise Exception()
...
class Shell:
    def __init__(self, ghost: 'GhostSqlite', db_path_local: str):
        ...
        if os.path.isfile(db_path_local):
            ghost.load_state_sqlite(db_path_local)
    ...
    def save(self):
        ...
        self.ghost.save_state_sqlite(self.db_path_local)
    ...
    def put_stimulus(self, content: str):
        """Public method to send a user message to the Shell's processing queue."""
        stim = Stimulus(source=self.ghost.config.user_name, content=content, stimulus_type=StimulusType.UserMessage)
        self.input_queue.put(stim)
        self.last_user_interaction_time = time.time()
    ...
    def _run_engagement_strategist(self):
        ...
        try:
            ...
            # Get facts about the user from the Ghost's memory
            user_facts_list = [
                k.content for k in self.ghost.all_declarative_facts
                if self.ghost.config.user_name in k.content
            ]
            user_profile = "\n- ".join(user_facts_list) if user_facts_list else "No specific facts known yet."
            ...
            **Task:** 
            ...
            _, calls = self.ghost.llm_manager.completion_tool(
...
def run_shell(interactive: bool, db_path: str | None = None, cmd: str = None):
    config = GhostConfig()
    ghost = GhostSqlite(config)

...
{target_name} is """
    ...
    def _generate_temporal_summary(self, temporal_cluster_knoxel: MemoryClusterKnoxel) -> str:
        ...
        for cluster in included_clusters:
            if cluster.cluster_type == ClusterType.Temporal and cluster.content:
                ...
                if event_knoxels:
                    ...
                    topic_lines = [f"{getattr(k, 'source', '?')}: {getattr(k, 'content', '')}..." for k in sorted(event_knoxels, key=lambda x: x.id)[:3]]
                    summary_input_lines.append(f"Conversation Topic between {cluster.timestamp_world_begin.strftime('%H:%M')} - {cluster.timestamp_world_end.strftime('%H:%M')}:\n  " + "\n  ".join(topic_lines))
        ...
        sysprompt = "You are an expert in literate and literal analysis, you help with stories and book reports and stuff like that." \
            "Your specific task is to assist the user in summarizing pieces of dialoge in a story. The summary should include a neutral description of what the characters talk about and how they feel. Don't make up any stuff and only focus on what's in the story."
        ...
        final_summary = self.ghost._call_llm(sysprompt=sysprompt, prompt=prompt)  # Assuming _call_llm exists on Ghost
...
class GhostSqlite(Ghost):
    ...
    def save_state_sqlite(self, filename: str):
        ...
        try:
            bak_path = filename + f".tick{self.current_tick_id - 1}.db"
